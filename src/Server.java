...
public class Server {
    ...
    HashMap<String, ArrayList<Guest>> roomHashMap; //방 목록 맵(방이름, 방에 속한 클라이언트 목록)


    void initNet() throws Exception {

        roomHashMap = new HashMap<String, ArrayList<Guest>>();
...
//            addGuest(guestThread); //새로운 게스트 객체를 로비 사용자 리스트에 추가한다
    ...
    void checkIfRoomExists(String roomName, Guest guest) throws Exception{

        if(roomHashMap.get(roomName) == null){ //이런 이름의 방이 존재하지 않는다면

            consoleLog(roomName +" does not exist. Create one.");

            addRoom(roomName, guest); //방을 새로 생성한다
            guest.sendMsg("serverMsg/A room is created."); //입장 메시지 발송
            broadcastRoomInfo(roomName); //방 정보 업데이트

        }else{ //이미 해당 이름의 방이 있다면

            consoleLog(roomName +" already exists.");

            //이미 참여중인 방인지 확인
            boolean isAlreadyJoinedRoom = false;
            int index;

            ArrayList<Guest> listOfGuestsInTheRoom = roomHashMap.get(roomName);
            ...
            if(isAlreadyJoinedRoom){
                ...
                consoleLog(guest.username+" has already joined "+roomName);

                broadcastToRoomExceptMe(roomName, "return/"+guest.username+" is back.", guest.id); //다른 사람에게 보내는 메시지
                guest.sendMsg("myReturn/Welcome back.");//나에게 보내는 메시지
                broadcastRoomInfo(roomName);

            }else{ //새로 가입한 방이라면

                consoleLog(guest.username+" is new to "+roomName);

                //join/welcome(나) --- join/oo entered the room(다른사람)
                addGuestToRoom(roomName, guest); //기존 방에 해당 유저를 추가한다
                broadcastToRoomExceptMe(roomName, "join/"+guest.username+" entered the room.", guest.id); //다른 사람에게 보내는 입장 메시지
...
//    }
    ...
    void removeRoom(String roomName){
        ...
        if(roomHashMap.get(roomName).size()==0){ //방에 아무도 없는 것을 확인하고
    ...
    void removeGuestFromRoom(String roomName, Guest guest) throws Exception{
        ...
        roomHashMap.get(roomName).remove(guest);
        ...
        broadcastToRoom(roomName, "out/"+guest.id+"/"+guest.username);
    }
    ...
    void addGuestToRoom(String roomName, Guest guest) {

        //방 목록 맵에 저장되어 있던, 이 방의 손님 리스트를 참조한다
        ArrayList<Guest> listOfGuestsInTheRoom_copied = roomHashMap.get(roomName);

        //그 리스트에 손님을 추가한다
        listOfGuestsInTheRoom_copied.add(guest);

        //방 이름과 손님 수를 서버 콘솔에 출력한다
        consoleLog("RoomName : " + roomName + ", Number of Guests : " + listOfGuestsInTheRoom_copied.size());
    }
    ...
    void addRoom(String roomName, Guest guest) {
        ...
        roomHashMap.put(roomName, listOfGuestsOfNewRoom);

        consoleLog("Name of the new Room :" + roomName);
        consoleLog("Number of Guests(It must be '1') :" + listOfGuestsOfNewRoom.size());
    }


    //각 방의 인원을 반환하는 메소드
    String getNumberOfGuestsInEachRoom(Set<String> roomList) throws Exception{

        StringBuffer buffer_numberOfGuestsInEachRoom = new StringBuffer("roomnum/"); //방에 사람수

        //각 방의 인원을 한 문장으로 나타냄
        for(String roomName : roomList){
            buffer_numberOfGuestsInEachRoom.append(roomHashMap.get(roomName).size()+"/");
        }
...
//    }
    ...
    void broadcastToRoom(String roomName, String msg) throws Exception {
    ...
    void broadcastToRoomExceptMe(String roomName, String msg, String userId) throws Exception {
...
//    }
    ...
    //특정 방의 정보를 방송한다
    //room_guestList/방이름/참여자 수/user1, user2, user3 -- 이런 식으로
    void broadcastRoomInfo(String roomName) throws Exception {

        ArrayList<Guest> guestListOfTheRoom = roomHashMap.get(roomName);
        ...
        broadcastToRoom(roomName, buffer_roomInfo.toString());
    }
